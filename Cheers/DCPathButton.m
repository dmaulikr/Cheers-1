...
- (CAAnimationGroup *)foldAnimationFromPoint:(CGPoint)endPoint
    ...
    rotationAnimation.values = @[@(0.0), @(M_PI * 0.5), @(M_PI), @(M_PI * 1.5), @(M_PI * 2)];
    rotationAnimation.duration = 0.2f;
    rotationAnimation.keyTimes = @[@(0.0), @(.25), @(.5), @(0.75), @(1.0)];
...
- (void)pathCenterButtonBloom {
    ...
    if (self.allowCenterButtonRotation) {
        [UIView animateWithDuration:0.5
                         animations:^{
                             _pathCenterButton.transform = CGAffineTransformMakeRotation(-2 * M_PI);
                         }];
    }
...
- (void)itemButtonTapped:(DCPathItemButton *)itemButton {
    ...
    if ([_delegate respondsToSelector:@selector(pathButton:clickItemButtonAtIndex:)]) {
        ...
        self.selectedButton = self.itemButtons[itemButton.index];

        [_pathCenterButton setImage:_selectedButton.itemImage forState:UIControlStateNormal];
        [_pathCenterButton setImage:self.selectedButton.itemImage forState:UIControlStateHighlighted];
        [UIView animateWithDuration:0.01f
                         animations:^{
                             _pathCenterButton.transform = CGAffineTransformMakeScale(4, 4);
                         }];
        ...
        [UIView animateWithDuration:0.01f
                         animations:^{
                             self.selectedButton.transform = CGAffineTransformMakeScale(3, 3);
                             self.selectedButton.alpha = 0.0f;
                         }];
        
        // Excute the dismiss animation when the item is unselected
        //
        for (int i = 0; i < self.itemButtons.count; i++) {
            if (i == self.selectedButton.index) {
                continue;
            }
